            ...
            <div class="card-body">
                <data-table :resource="resource" :typeUser="typeUser" :soapCompany="soapCompany">
                    ...
                    <tr slot-scope="{ index, row }" :class="{ anulate_color : row.state_type_id == '11' }">
                        ...
                        <td class="text-right">
                            <label v-if="row.documents.length > 0" :key="'doc_payment_'+index"
                                   v-text="calculatePayments(row.documents)"></label>
                            <label v-if="row.sale_notes.length > 0" :key="'sale_note_payment_'+index"
                                   v-text="calculatePayments(row.sale_notes)"></label>
                        </td>
                        ...
                        <td class="text-right">
                            ...
                            seller_can_generate_cpe === true &&
                             soapCompany != '03'" type="button" class="btn waves-effect waves-light btn-xs btn-info"
                                    @click.prevent="clickOptions(row.id)">Generar comprobante
                            </button>
                            ...
                            <a :href="`/dispatches/create_new/order_note/${row.id}`"
...
import StateType from "./components/StateType.vue"
import ChangeStateType from "./components/ChangeStateType.vue"
