...
class ClientController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $certificate_date_of_issue = null;
            $certificate_date_of_due = null;
...
//            }
            ...
            if ($temp_path) {
                try {
                    $temp_path = $request->input('temp_path');
                    $filename = $request->input('certificate');
                    $password = $request->input('password_certificate');
                    ...
                    if ($extension === 'pem') {
                        $pem_content = file_get_contents($temp_path);
                    ...
                    $new_filename = 'certificate_' . $request->input('number') . '_' . date('Ymd_His') . '.pem';
                    if (!file_exists(storage_path('app' . DIRECTORY_SEPARATOR . 'certificates'))) {
                        mkdir(storage_path('app' . DIRECTORY_SEPARATOR . 'certificates'));
                    }
                    file_put_contents(storage_path('app' . DIRECTORY_SEPARATOR . 'certificates' . DIRECTORY_SEPARATOR . $new_filename), $pem_content);

                    $name_certificate = $new_filename;
                    $certificate_date_of_issue = $date_of_issue;
                    $certificate_date_of_due = $date_of_due;
            ...
            DB::connection('tenant')
                ...
                ->update([
                    ...
                    'certificate_date_of_issue' => $certificate_date_of_issue,
                    'certificate_date_of_due' => $certificate_date_of_due,
                ]);
    ...
    public function store(ClientRequest $request)
    {
        ...
        if ($temp_path) {
            try {
                ...
                $new_filename = 'certificate_' . $request->input('number') . '_' . date('Ymd_His') . '.pem';
//                $new_filename = 'certificate_' . 'admin_tenant' . '.pem';
                if (!file_exists(storage_path('app' . DIRECTORY_SEPARATOR . 'certificates'))) {
                    mkdir(storage_path('app' . DIRECTORY_SEPARATOR . 'certificates'));
                }
                file_put_contents(storage_path('app' . DIRECTORY_SEPARATOR . 'certificates' . DIRECTORY_SEPARATOR . $new_filename), $pem_content);
